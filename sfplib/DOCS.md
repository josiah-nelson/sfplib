...
All data is stored in `/config/sfplib/`:
...
**Solutions:**
1. Check `/config/sfplib/` has write permissions
...
```bash
# Backup database
cp /config/sfplib/sfp_library.db /backup/sfp_library_$(date +%Y%m%d).db

# Restore
cp /backup/sfp_library_20250101.db /config/sfplib/sfp_library.db
```
...
- All data stored locally in `/config/sfplib/`
