...
Complete guide for deploying SFPLiberate as a Home Assistant add-on.

---

## Deployment Mode

SFPLiberate is **exclusively** deployed as a **Home Assistant add-on**. Previous standalone Docker and Appwrite cloud deployments have been archived.

**Why HA-first:**
- Native Bluetooth integration (no browser compatibility issues)
- Simpler architecture (no ESPHome proxy complexity)
- Better UX (auto-discovery, ingress, backups)
- Target audience is HA users
...
- **Home Assistant OS** or **Supervised** (2023.8.0+)
- Bluetooth adapter (built-in or USB dongle)
- SFP Wizard device (firmware v1.0.10+)
...
2. **Install add-on:**
   - Find "SFPLiberate" in add-on store
   - Click **Install**
...
make db-backup && \
  aws s3 cp backups/ s3://my-bucket/ha-backups/ --recursive
```
